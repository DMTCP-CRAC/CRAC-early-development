...
mtcp_writememoryareas(int fd)
{
  ...
  // whiteAreas = (ProcMapsArea *)malloc(1000 * sizeof(ProcMapsArea));
  ...
  while (procSelfMaps->getNextArea(&area)) {
    ...
    JNOTE("before calling to skip") ((void *)area.addr) ((void *)area.endAddr) (area.size);
    int skip = dmtcp_skip_memory_region_ckpting(&area, fd, stack_was_seen);
    JNOTE("after calling to skip") ((void *)area.addr) ((void *)area.endAddr) (area.size) (skip);
    if (skip != 0) {
      // JNOTE("Skipping region as requested by the plugin") (area.addr)
      //     (area.endAddr) (area.size);
      continue;
    }
